...
def cross_validation (df, model_name):
    ...
    if model_name.lower() == 'mlp':
        ...
        for fold, (train, val) in enumerate(gkf.split(feat, alvo, group)):
            ...
            model = MLP(input, 2)
            model.train_loop (train_dataloader, int(sys.argv[2]), 'cpu')
        ...
        if model_name.lower() == 'svm':
            model = SVC(kernel=sys.argv[2], C=float(sys.argv[3]), gamma='scale')

        elif model_name.lower() == 'rf':
            try:
                model = RandomForestClassifier(n_estimators=int(sys.argv[2]), max_depth = int(sys.argv[3]))
            except:
                model = RandomForestClassifier(n_estimators=int(sys.argv[2]), max_depth = None)

        else:
            model = KNeighborsClassifier(n_neighbors=int(sys.argv[2]))
